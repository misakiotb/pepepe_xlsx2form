...
この拡張機能は、ユーザーのプライバシーとセキュリティを最優先に設計されています。

- **最小限の権限**: 拡張機能は、ユーザーがアイコンをクリックして操作した、現在開いているタブ（`activeTab`）に対してのみ一時的なアクセス許可を要求します。ユーザーが意図しないページで勝手に動作することはありません。
- **データはローカルで完結**: ユーザーがアップロードしたExcelファイルやマッピング設定は、すべてお使いのブラウザ内で処理が完結します。これらのデータが外部のサーバーに送信されることは一切ありません。
- **コードの透明性**: すべてのソースコードはGitHubで公開されており、どのような処理が行われているかを確認できます。
...
-   対応するフォーム要素の種類を増やしたい場合や、より複雑なデータ加工・入力ルールが必要な場合は、`popup.js`内の`chrome.scripting.executeScript`で実行されている関数を修正してください。ここが、Webページ上で実際にフォーム操作を行う中心部分です。
